...
const qPokoyDevVersion='dev-2026.09.25.29';
...
window.renderIncomes=function renderIncomes(filteredData=null){
  ...
  incomeList.innerHTML=header+
    rows.map(item=>{
      ...
      const actions=`
        <button class="edit-income" data-id="${escapeHtml(item.id)}" type="button" title="Редактировать" aria-label="Редактировать"><svg class="history-action-pencil" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zm17.71-10.21a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg><span class="history-action-label" style="display:none">Изменить</span></button>
