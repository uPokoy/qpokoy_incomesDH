...
const qPokoyDevVersion='dev-2026.09.26.57';
...
window.renderIncomeAnalytics=function(){
  ...
  if(annualHeroBarsEl){
    ...
    annualHeroBarsEl.innerHTML=a.monthTotals.map((value,index)=>{
      ...
      return `<span class="annual-total-bar${value>0?'':' is-zero'}${index===selectedPeriod.month?' selected':''}" data-month="${index}" role="button" tabindex="0" style="--bar-height:${height.toFixed(1)}%" title="${fullNames[index]}: ${formatMoney(value)}" aria-label="Показать ${fullNames[index]} ${a.year}: ${formatMoney(value)}"></span>`;
    }).join('');
  }
