(function qPokoyReportPrintInit(){
  ...
  function buildReportHtml(report,mode,period,range){
    ...
    return '<!doctype html><html lang="ru"><head><meta charset="utf-8">'+
      ...
      '<style>'+
      '@page{size:A4;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff;color:#111827;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:13px;line-height:1.4}'+
      'body{padding:22px;margin:0;background:#fff}.report-document{max-width:920px;margin:0 auto}.report-head{width:82%;display:flex;justify-content:space-between;align-items:flex-start;gap:24px;margin:0 auto 18px}.period{font-size:20px;font-weight:750;line-height:1.15;color:#111827}.report-meta{text-align:right;color:#6b7280;font-size:11px;line-height:1.5;white-space:nowrap}.summary{width:82%;display:grid;grid-template-columns:1.45fr .8fr .8fr;margin:0 auto 18px;background:#f5f7fa;border:1px solid #edf0f3;border-radius:9px;overflow:hidden}.metric{min-height:70px;padding:12px 15px;display:flex;flex-direction:column;justify-content:center}.metric+.metric{border-left:1px solid #dde2e8}.metric span{margin:0 0 6px;color:#6b7280;font-size:11px}.metric strong{font-size:23px;line-height:1;font-weight:750;letter-spacing:-.02em}.metric:not(:first-child) strong{font-size:20px}.income-table{width:82%;margin:0 auto;table-layout:fixed;border-collapse:separate;border-spacing:0;border:1px solid #e3e8ee;border-radius:8px;overflow:hidden;font-size:14px}thead{display:table-header-group}th{background:#f3f4f6;color:#4b5563;font-size:12px;font-weight:650;text-align:left}th,td{padding:8px 9px;border-right:1px solid #edf0f3;border-bottom:1px solid #e7ebf0;vertical-align:top}th:last-child,td:last-child{border-right:0}tbody tr:last-child td{border-bottom:0}.date{width:18%;text-align:center;white-space:nowrap}.category{width:23.8%;text-align:center}.description{width:38.2%;white-space:normal;overflow-wrap:anywhere;word-break:normal}.number{width:20%;text-align:right;white-space:nowrap;font-weight:650}.total-row td{background:#f8fafc;font-weight:750;border-top:1px solid #e1e6ec}.total-row .total-inline{text-align:right;white-space:nowrap;font-size:14px}.total-row .total-inline span{margin-right:8px}.total-row .total-inline strong{font-size:14px}.report-actions{position:fixed;left:50%;transform:translateX(-50%);width:min(754px,calc(82vw - 36px));bottom:20px;z-index:20;display:flex;align-items:center;justify-content:flex-end;gap:8px}.report-document{padding-bottom:64px}.report-actions button{padding:11px 15px;border-radius:9px;font:600 13px inherit;cursor:pointer}.report-print-btn{border:1px solid #d7dde5;background:#fff;color:#374151;box-shadow:0 4px 14px rgba(15,23,42,.08)}.report-save-btn{border:0;background:#2563eb;color:#fff;box-shadow:0 5px 16px rgba(37,99,235,.18)}.report-save-btn:disabled{opacity:.6;cursor:wait}tr{break-inside:avoid;page-break-inside:avoid}.paged-export-root{width:210mm;padding:0;background:#fff;isolation:isolate}.pdf-export-root{position:relative;margin:0 auto}.print-export-root{position:absolute;left:0;top:0;margin:0;visibility:hidden;pointer-events:none}.paged-page{width:210mm;height:295mm;padding:13mm 12mm;box-sizing:border-box;background:#fff;position:relative;overflow:hidden;break-after:page;page-break-after:always;break-inside:avoid;page-break-inside:avoid}.paged-page:last-child{break-after:auto;page-break-after:auto}.paged-page-content{height:100%;overflow:hidden}.paged-page .report-head,.paged-page .summary,.paged-page .income-table{width:100%;margin-left:0;margin-right:0}.paged-page .income-table{border-radius:8px;overflow:hidden}.paged-page-number{position:absolute;right:12mm;bottom:5mm;font-size:9px;font-weight:500;color:#9ca3af}@media print{html,body{margin:0;padding:0;background:#fff}.report-document{display:none}.report-actions{display:none}.pdf-export-root{display:none}.print-export-root{position:static;visibility:visible;pointer-events:auto;display:block;width:210mm;margin:0}.print-page{display:block}.print-page .income-table{border-radius:8px;overflow:hidden}*{-webkit-print-color-adjust:exact;print-color-adjust:exact}}'+
      '</style></head><body>'+
      ...
      '</tbody></table>'+
      '</main>'+
      '<div class="report-actions"><button type="button" class="report-print-btn" onclick="printPreparedReport()">Печать</button><button type="button" class="report-save-btn" id="saveReportPdfBtn" onclick="saveReportPdf()">Сохранить PDF</button></div>'+
      '<script src="https://cdn.jsdelivr.net/npm/html2pdf.js@0.10.1/dist/html2pdf.bundle.min.js"><\/script>'+
      '<script>'+
      'const qPokoyPdfFileName='+JSON.stringify(pdfFileName)+';'+
            'function buildPagedExport(rootClass,pageClass){'+
        'const source=document.getElementById("reportDocument");'+
        'const sourceTable=source&&source.querySelector(".income-table");'+
        'if(!source||!sourceTable)return null;'+
        'const allRows=Array.from(sourceTable.querySelectorAll("tbody tr"));'+
        'const totalRow=allRows.find(function(row){return row.classList.contains("total-row");})||null;'+
        'const dataRows=allRows.filter(function(row){return !row.classList.contains("total-row");});'+
        'const root=document.createElement("div");root.className="paged-export-root "+rootClass;document.body.appendChild(root);'+
        'let pageNumber=0;'+
        'function createPage(first){'+
          'pageNumber+=1;'+
          'const page=document.createElement("section");page.className="paged-page "+pageClass;'+
          'const content=document.createElement("div");content.className="paged-page-content";page.appendChild(content);'+
          'if(first){const head=source.querySelector(".report-head");const summary=source.querySelector(".summary");if(head)content.appendChild(head.cloneNode(true));if(summary)content.appendChild(summary.cloneNode(true));}'+
          'const table=document.createElement("table");table.className="income-table paged-page-table";'+
          'const thead=sourceTable.querySelector("thead");if(thead)table.appendChild(thead.cloneNode(true));'+
          'const tbody=document.createElement("tbody");table.appendChild(tbody);content.appendChild(table);'+
          'const footer=document.createElement("div");footer.className="paged-page-number";footer.textContent=String(pageNumber);page.appendChild(footer);'+
          'root.appendChild(page);return {page:page,content:content,table:table,tbody:tbody};'+
        '}'+
        'function fitsPage(state){return state.table.getBoundingClientRect().bottom<=state.content.getBoundingClientRect().bottom-2;}'+
        'let state=createPage(true);'+
        'dataRows.forEach(function(row){const clone=row.cloneNode(true);state.tbody.appendChild(clone);if(!fitsPage(state)&&state.tbody.children.length>1){state.tbody.removeChild(clone);state=createPage(false);state.tbody.appendChild(clone);}});'+
        'if(totalRow){const clone=totalRow.cloneNode(true);state.tbody.appendChild(clone);if(!fitsPage(state)&&state.tbody.children.length>1){state.tbody.removeChild(clone);state=createPage(false);state.tbody.appendChild(clone);}}'+
        'return root;'+
      '}'+
      'function buildPdfExport(){return buildPagedExport("pdf-export-root","pdf-page");}'+
      'function buildPrintExport(){return buildPagedExport("print-export-root","print-page");}'+
      'function printPreparedReport(){'+
        'const source=buildPrintExport();'+
        'if(!source){window.print();return;}'+
        'let cleaned=false;'+
        'function cleanup(){if(cleaned)return;cleaned=true;try{source.remove();}catch(e){}}'+
        'window.addEventListener("afterprint",cleanup,{once:true});'+
        'setTimeout(function(){try{window.print();}finally{setTimeout(cleanup,0);}},60);'+
      '}'+
      'function saveReportPdf(){'+
        ...
        'const source=buildPdfExport();'+
        'if(!source){button.disabled=false;button.textContent="Сохранить PDF";alert("Не удалось подготовить PDF.");return;}'+
        'const options={margin:0,filename:qPokoyPdfFileName,image:{type:"jpeg",quality:.98},html2canvas:{scale:2,useCORS:true,backgroundColor:"#ffffff",logging:false,scrollX:0,scrollY:0},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["css"]}};'+
        'html2pdf().set(options).from(source).save().then(function(){source.remove();button.disabled=false;button.textContent="Сохранить PDF";}).catch(function(){source.remove();button.disabled=false;button.textContent="Сохранить PDF";alert("Не удалось сохранить PDF. Используйте кнопку «Печать».");});'+
      '}'+
      '<\/script>'+
      '</body></html>';
  }
